...
void Board::draw(sf::RenderTarget& target, sf::RenderStates states) const {
        ...
        for (const auto& c : cells) {
            ...
            if (c.isHighlighted){ cell.setFillColor(Constants::highlightColor); }
            else if (c.isSelected) {cell.setFillColor(Constants::selectedColor); }
            else{ cell.setFillColor(sf::Color::White); }

            target.draw(cell, states);

            sf::Text cellIDText(Constants::normalFont, std::to_string(c.cellID));
            cellIDText.setPosition({col * 60.f + 45.f, row * 60.f + 45.f});
            cellIDText.setFillColor(Constants::gray);
            cellIDText.setCharacterSize(10);

            target.draw(cellIDText, states);

            sf::Text rightCellValue(Constants::normalFont, std::to_string(c.rightValue));
            ...
            if (c.occupyingValue != std::nullopt) {
                sf::Text cellOccupyingValue(c.isSelected ? Constants::boldFont : Constants::normalFont, std::to_string(c.occupyingValue.value()));
        ...
        for (int i = 0; i<10; i++) {
            ...
            line.setFillColor(Constants::thinOutlineColor);
        ...
        for (int i = 0; i<10; i++) {
            ...
            line.setFillColor(i%3 == 0 ? Constants::thickOutlineColor : Constants::thinOutlineColor);
            target.draw(line, states);
        }
        ...
        for (int i = 0; i<4; i++) {
            ...
            line.setFillColor(Constants::thickOutlineColor);
            target.draw(line, states);
        }
